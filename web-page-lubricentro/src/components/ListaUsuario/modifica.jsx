...
import { useParams, useNavigate } from 'react-router-dom';
import { useDispatch, useSelector } from 'react-redux';
import { updateUser, deleteUser, fetchUsers } from '../redux/actions/userActions';
import './modifica.css';
...
const EditUser = () => {
  ...
  // Cargar usuarios del backend si aún no están disponibles
  useEffect(() => {
    if (!user) {
      dispatch(fetchUsers());
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    alert('Información actualizada para el usuario: ' + userData.name);
    navigate('/user-list'); // Redirige a la lista de usuarios
  };
  ...
  if (!user) {
    return <div>Cargando usuario...</div>; // Mostrar un mensaje mientras se carga
  }
